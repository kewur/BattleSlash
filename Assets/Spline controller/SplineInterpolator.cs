...
public class SplineInterpolator : MonoBehaviour
{
	...
	private SplineController mSplineController;
	...
	public Vector3 GetHermiteInternal(int idxFirstPoint, float t)
	{
		...
		float tension = 0.0f;	// 0.5 equivale a catmull-rom
	...
	{
		Transform[] nodes = mSplineController.GetTransforms();
		
		if(nodes.Length == mNodes.Count)
		{
			for(int i = 0 ; i < mNodes.Count ; i++)
			{
				
				mNodes[i].Point = nodes[i].transform.position;
